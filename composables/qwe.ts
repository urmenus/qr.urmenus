...
export const useQwe = async (card: Label, max: number) => {
  const zip = new JSZip()
  const cardFolder = zip.folder('cards')

  console.log('getArrayForTables(max): ', getArrayForTables(max))
